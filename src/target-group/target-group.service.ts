...
export class TargetGroupService {
    ...
    createTargetGroup(name: string, filter: FilterModel): Promise<TargetGroup> {
        const id = uuid();
        const query = `
        MATCH (c1:Client) ${this.createQuery(filter)};
        RETURN c1
        `
    ...
    createQuery(filter: FilterModel): string {
        return filter.rules.map(r => {
            return "c1." + r.name + r.operator + r.value;
        })

        // if (filter.relatedEntities && filter.relatedEntities.length) {
        //     where.push(`(c)<-[:FAVORITED]-({username: $favorited})`)
        // }
        return `
        MATCH (c1:Client) ${where.length ? 'WHERE ' + where.join(' AND ') : ''}
        // MATCH (c2:Client) WHERE c2.lastName = "Doe"
        // MATCH (c1)-[:parent]->(c2)
        return c1
        `;
    }

    // create(title: string, description: string, body: string, tagList: string[]): Promise<TargetGroup> {
    //     return this.neo4jService.write(`
    ...
    // }
    //
    // find(slug: string): Promise<TargetGroup | undefined> {
    //     return this.neo4jService.read(`
    ...
    // }
    //
    // update(slug: string, updates: Record<string, any>): Promise<TargetGroup> {
    ...
    // }
    //
    // favorite(slug: string): Promise<TargetGroup> {
    //     return this.neo4jService.write(`
    //         MATCH (a:Article {slug: $slug})
    //         MATCH (u:User {id: $userId})
    //
    ...
    // }
    //
    // unfavorite(slug: string): Promise<TargetGroup> {
    //     return this.neo4jService.write(`
    //         MATCH (a:Article {slug: $slug})
    //         MATCH (u:User {id: $userId})
    //
